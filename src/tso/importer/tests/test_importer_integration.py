...
class TestDataImporterIntegration:
    ...
    def test_observation_get_all(self):
        observations = data_importer.get_all_observations(db_config=get_db_config())
    ...
    def test_observation_get_with_constraint(self):
        ...
        all_observations = data_importer.get_all_observations(db_config=get_db_config())

        assert len(observations) <= len(all_observations)

    def test_observation_get_with_constraint_with_default_values(self):
        observations = data_importer.get_observations(db_config=get_db_config())
        all_observations = data_importer.get_all_observations(db_config=get_db_config())

        assert len(observations) == len(all_observations)

    def test_getting_exposure_counts_per_id(self):
        exposures_count_by_id = data_importer.get_exposure_counts_per_observation_id(db_config=get_db_config())
        print(exposures_count_by_id)

    def test_observation_blocks_should_have_their_exposure_set(self):
        result_from_all = data_importer.get_all_observations(db_config=get_db_config())
        result_from_filters = data_importer.get_observations(db_config=get_db_config())
